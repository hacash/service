...
func (api *DeprecatedApiService) createTxAndCheckOrCommit(w http.ResponseWriter, value []byte) {
	...
	tx, readability, e := createTransactionFromJson(kernel.StateRead(), value)
	if e != nil {
		w.Write([]byte(fmt.Sprintf(`{"error":"Create transaction error: %s"}`, e.Error())))
		return
	}

	ckstate, e := kernel.CurrentState().ForkSubChild()
	if e != nil {
		w.Write([]byte(fmt.Sprintf(`{"error":"Check Tx ForkSubChild error: %s"}`, e.Error())))
		return
	}
	...
	var txres = fmt.Sprintf(`,"txhash":"%s","txbody":"%s","description":["%s"]`,
		hashnofee.ToHex(),
	...
	if e != nil {
		w.Write([]byte(fmt.Sprintf(`{"success":false%s,"error":"%s"}`, txres, e.Error())))
		return
	}
	...
	if e1 == nil && e2 == nil {
		...
		if e1 == nil && e2 == nil && len(pubkey) == 33 && len(signature) == 64 {
			...
			if e != nil {
				...
				w.Write([]byte(fmt.Sprintf(`{"success":false%s,"error":"%s"}`, txres, e1.Error())))
				return
			}
		}
	}
 ...
 */
func createTransactionFromJson(state interfaces.ChainStateOperationRead, jsonvalue []byte) (tx *transactions.Transaction_2_Simple, readability []string, err error) {
	...
	if e != nil {
		err = fmt.Errorf("Tx fee set error: %s", e.Error())
		return
	}
	// addr
	...
	// create tx
	tx, e = transactions.NewEmptyTransaction_2_Simple(*madr)
	if e != nil {
		err = e
		return
	}
